...
use scryer_application::{AppResult, RequestRuleSetRepository};
...
impl RequestRuleSetRepository for RequestRuleSetStore {
    ...
    /// Unlike the maintenance twin there is no disarm riding along: a request
    /// rule votes and never executes, so no acknowledgement is attached to the
    /// revision it was granted against.
    async fn add_revision(
        ...
        SqlRuntime::run_in_transaction(&self.datastore, "add_request_rule_revision", move |tx| {
            let revision_args = revision_args.clone();
    ...
    async fn update_rule_set_evaluation_mode(
        ...
        execute_write(
            &self.datastore,
            "update_request_rule_set_evaluation_mode",
            "UPDATE request_rule_sets
    ...
    async fn delete_rule_set(&self, id: &str) -> AppResult<()> {
        execute_write(
            &self.datastore,
            "delete_request_rule_set",
            "DELETE FROM request_rule_sets WHERE id = {}",
...
fn row_to_rule_set(row: &SqlRow) -> AppResult<RequestRuleSet> {
    Ok(RequestRuleSet {
        id: row.text("id")?,
        name: row.text("name")?,
...
fn row_to_revision(row: &SqlRow) -> AppResult<RequestRuleRevision> {
    Ok(RequestRuleRevision {
        id: row.text("id")?,
...
/// DDL default carries verbatim; parsing it leniently keeps such a row readable
