...
fn decode_grab_result(
...
) -> AppResult<Option<scryer_application::ResolvedDownloadArtifact>> {
    use scryer_application::ResolvedDownloadArtifact;
    let response = match result {
        PluginResult::Ok(response) => response,
        PluginResult::Err(error)
            if error.code == scryer_plugin_sdk::PluginErrorCode::Unsupported =>
        {
            return Ok(None);
        }
        PluginResult::Err(error) => {
            return Err(AppError::Repository(format!(
                "indexer download resolution: plugin error {:?}: {}",
                error.code, error.public_message,
            )));
        }
    };
    ...
    if is_valid_magnet_uri(payload.url.trim()) {
        return Ok(Some(ResolvedDownloadArtifact::Magnet {
            uri: payload.url.trim().to_string(),
            info_hash_hint: scryer_application::extract_magnet_info_hash(&payload.url)
                .or(payload.info_hash_hint),
        }));
    }
    if payload.body.is_empty() {
        return Err(AppError::Repository(
            "indexer grab returned an empty artifact".into(),
        ));
    }
    let head = String::from_utf8_lossy(&payload.body[..payload.body.len().min(4096)]);
    if head.contains("<nzb") {
    ...
    if payload.body.first() != Some(&b'd') || payload.body.last() != Some(&b'e') {
        return Err(AppError::Repository(
            "indexer grab returned neither NZB nor torrent data".into(),
        ));
    }
...
impl IndexerClient for WasmIndexerClient {
    async fn resolve_download(
        ...
        self.resolve_download_with_cancellation(download_url, &CancellationToken::new())
            .await
    }
    ...
    async fn resolve_download_with_cancellation(
        ...
        if cancellation.is_cancelled() {
            return Err(AppError::canceled("indexer grab canceled"));
        }
        let command = PluginIndexerCommand::Action(PluginActionRequest {
        ...
        if let Some(indexer) = self.component.as_ref() {
            ...
            let result = match self
                .invoke_component(command, "indexer_grab", Some(cancellation))
                .await
            {
                Ok(Some(PluginIndexerCommandResult::Action(result))) => decode_grab_result(result),
                Ok(_) => Err(AppError::Repository(
                    "indexer grab returned the wrong result".into(),
                )),
                Err(error) => Err(error),
            };
            let result = match result {
        ...
        if let Some(indexer) = self.command.as_ref() {
            ...
            let result = match self
                .invoke_command(command, "indexer_grab", Some(cancellation))
                .await
            {
                Ok(Some(PluginIndexerCommandResult::Action(result))) => decode_grab_result(result),
                Ok(_) => Err(AppError::Repository(
        ...
        // Legacy providers have no authenticated grab contract. Their optional
        // action ABI predates the payload above and is not interchangeable.
        Ok(None)
    }
...
mod tests {
    ...
    fn grab_plugin_errors_only_fall_back_for_unsupported() {
        for (code, fallback) in [
            ...
            (scryer_plugin_sdk::PluginErrorCode::Temporary, false),
        ] {
            let result = decode_grab_result(PluginResult::Err(PluginError {
                code,
                public_message: "provider failure".into(),
                debug_message: None,
    ...
    #[test]
    fn grab_decodes_nzb_and_magnet_and_rejects_empty_artifacts() {
        let decode =
            |payload| decode_grab_result(PluginResult::Ok(PluginActionResponse { payload }));
        let nzb = b"<?xml version=\"1.0\"?><nzb></nzb>";
        assert!(matches!(decode(serde_json::json!({
            "url": "https://indexer.example/grab/1", "body": nzb.to_vec(),
        })).unwrap(), Some(scryer_application::ResolvedDownloadArtifact::Nzb { bytes, .. }) if bytes == nzb));
        let magnet = "magnet:?xt=urn:btih:abcdef0123456789abcdef0123456789abcdef01";
        assert!(
            matches!(decode(serde_json::json!({"url": magnet})).unwrap(),
            Some(scryer_application::ResolvedDownloadArtifact::Magnet { uri, .. }) if uri == magnet)
        );
        assert!(decode(serde_json::json!({"url": "https://indexer.example/login"})).is_err());
    }
