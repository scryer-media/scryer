//! End-to-end upgrade coverage for migrations 0204, 0205 and 0206.
//!
//! Each scenario drives the real migration runner over a database the real
//! catalog built at the pre-0204 state, because the behaviour worth pinning is
...
async fn pre_upgrade_pool() -> SqlitePool {
    ...
    .await
    .expect("pre-0204 migration fixture should apply");
    pool
}
...
async fn apply_upgrade(pool: &SqlitePool) {
    crate::migrations::run_migrations(pool, crate::MigrationMode::Apply)
        .await
        .expect("0204-0206 upgrade should apply");
    for version in [204, 205, 206] {
        let applied: i64 =
...
async fn postgres_upgrade_rekeys_roots_and_creates_the_location_tables() {
    let Some(url) = postgres_test_url() else {
        eprintln!(
            "skipping PostgreSQL 0204-0206 upgrade test; SCRYER_TEST_POSTGRES_URL is not set"
        );
        return;
    };
    ...
    let outcome = async {
        crate::postgres::replay_source_catalog_for_fresh_install(&pool, Some(PRE_UPGRADE_VERSION))
            .await
            .expect("pre-0204 postgres fixture should apply");
        ...
        let services =
            crate::postgres::PostgresServices::new_with_mode(&scoped_url, crate::MigrationMode::Apply)
                .await
                .expect("0204-0206 postgres upgrade should apply");
        drop(services);
