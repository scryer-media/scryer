//! Migration 0215 — synthetic stable root ids (FR-078, plan D1).
//!
...
//! functional dependency once, in one transaction, so that from 0215 onward the
...
//! matters: after 0215 no root's id is its path-derived id.
//!
//! ## Referents
//!
...
/// The invariant 0215 exists to establish: no root's id is a function of its path.
...
fn assert_no_path_derived_root_ids(roots: &[RootRow]) -> AppResult<()> {
    for root in roots {
        ...
        if root.id == root_folder_id_for_normalized_path(&normalized_path) {
            return Err(AppError::Repository(format!(
                "library root {} still carries a path-derived id after migration 0215",
                root.id
            )));
        }
    }
