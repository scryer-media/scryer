...
ALTER TABLE pending_releases
    ADD COLUMN release_age_unknown BOOLEAN NOT NULL DEFAULT FALSE;
...
SET release_identity = CASE
        ...
        ELSE
            ...
            lower(trim(release_title)) || ':' || lower(trim(COALESCE(published_at::text, 'unknown')))
    END,
    ...
    release_age_unknown = published_at IS NULL AND delay_until > added_at;
