...
impl AppUseCase {
    ...
    pub(crate) async fn resolve_release_search_subject_for_episode(
        ...
        // An interactive search asks for the same release names the automatic
        // lane does. Release groups number many titles by an order the catalog
        // does not follow — anime per cour, ordinary series by a TVDB alternate
        // order — so without these forms a manual search never mentions the
        // season and episode numbers the groups actually post under, and the
        // results it does return are whatever the bare title matched.
