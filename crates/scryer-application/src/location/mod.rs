...
//! | [`root_scope`] | The root-scoped planner for both branches of **Change root**: every-title accounting, unmanaged-content buckets, retirement ordering, FR-024's classification and FR-025 folder uniquing for the branch that folds into an existing root (FR-020–029, FR-087). |
//! | [`root_scope_execution`] | The root-scoped use case: both preview/start entry points, destination admissibility, and the one tail that retires the source location and then either flips the root's path or removes its configuration (FR-020, FR-022, FR-028, FR-087). |
