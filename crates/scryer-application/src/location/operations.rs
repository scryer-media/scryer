...
/// transfer, and consolidation when it lands — plans
...
struct RootMoveTitleOutcome {
    ...
    /// probes, from a title that actually moves files.
...
impl AppUseCase {
    ...
    pub async fn resume_location_operation(
        ...
        // ownership (FR-056): both are planned by the same planner, persisted
        // as the same `RootMoveExecutionPlan`, and walked by the same runner, so
        // they resume here. Every *other* type resumes through its own phase and
        // must not be run under these rules.
        //
    ...
    pub async fn run_root_move(
        ...
        // The mode is read off the persisted row rather than re-derived, so a
        // resumed run runs in the mode the user confirmed.
        let mode = row.mode;
    ...
    async fn execute_root_move(
        ...
        //
        // Bound here rather than boxed so the runner keeps borrowing a concrete
        // `&dyn TitleFileMover` for the whole run.
        ...
        let copy_mover = RootMoveFileMover::new(VerifiedCopier::new(), permissions.clone())
        ...
        let mover: &dyn crate::location::executor::TitleFileMover = &copy_mover;
