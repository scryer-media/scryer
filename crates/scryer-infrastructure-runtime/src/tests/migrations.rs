...
#[tokio::test]
async fn migration_0200_compacts_discovery_payloads_and_rekeys_recommendation_cards() {
    crate::spellfix::register_spellfix_auto_extension()
        .expect("spellfix auto-extension should register");
    let db = std::env::temp_dir().join(format!(
        "scryer_migration_0200_discovery_storage_{}.db",
        chrono::Utc::now().timestamp_micros()
    ));
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        ...
        .await
        .expect("migration test database should open");
    crate::migrations::replay_source_catalog_for_fresh_install(&pool, Some(199), true)
        .await
        .expect("0199 fixture schema should install");
...
#[tokio::test]
async fn migration_0200_postgres_uses_native_payload_and_timestamp_types() -> AppResult<()> {
    let Some(raw_url) = std::env::var("SCRYER_TEST_POSTGRES_URL")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
    ...
    let result = async {
        crate::postgres::replay_source_catalog_for_fresh_install(&pool, Some(199)).await?;
        sqlx::query(
            "INSERT INTO discovery_sync_runs (
                id, kind, status, trigger_source, region, language,
                raw_ack_json, created_at, updated_at
